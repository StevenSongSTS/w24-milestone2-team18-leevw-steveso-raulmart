...
from db_helper_functions import (
    create_finbert_tone_whole_article_sentiment_scores_table,
    drop_finbert_tone_whole_article_sentiment_scores_table,
    get_finbert_tone_whole_article_news_ids,
...
def gen_sentiments(sent_list):
    embedding = finbert_tokenizer(
        sent_list, padding=True, return_tensors="pt", max_length=512, truncation=True
    )
...
def iterative_sentiment_predictions(ticker: str):
    existing_scores = get_finbert_tone_whole_article_news_ids()[
        "fk_stock_news_id"
    ].to_list()
    ...
    for idx, chunk in groups:
        ...
        chunk[["fk_stock_news_id", "positive", "negative", "neutral"]].to_sql(
            "finbert_tone_whole_article_sentiment_scores",
            DB_URL,
            if_exists="append",
            index=False,
        )
